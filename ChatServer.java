...
class ServerThread extends Thread {
    ...
    // ChatServer 클래스의 객체를 멤버 변수로 선언, has-a 관계를 위함
    ChatServer cg;
    ...
    public ServerThread(ChatServer cg, Socket s) {
        /* cg = new ChatServer(); → 작성 불가, 서버가 두 번 가동되기 때문에 충돌이 일어남
    ...
    // run()_ServerThread -> broadCast(str)_ChatServer -> send(str)_ServerThread
    public void run() {
        try {
